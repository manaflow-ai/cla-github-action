...
The action also emits `api_result` as a small, stable result type for trusted downstream policy jobs. It is `success` when the action completes without a policy failure, `unsigned` when GitHub requests completed but one or more contributors still need to sign, `retryable_error` when a transport or transient GitHub API failure prevented a decision, and `error` for validation, authorization, malformed-response, or permanent API failures. Both error values fail the action and leave `cla_passed=false`; `retryable_error` is a classification only. The action does not retry a failed API write automatically because a lost response could have committed it. Its existing bounded contents-conflict recovery is separate: it reads the ledger again, revalidates the Pull Request and signing comment, and retries only an explicit 409 conflict.
...
| Name                  | Description |
| --------------------- | ----------- |
| `api_result` | Stable result type for trusted consumers: `success`, `unsigned`, `retryable_error`, or `error`. A retryable result identifies a transport or transient GitHub API failure and does not authorize a retry by itself. Failed API writes are not retried automatically; only an explicit 409 contents conflict uses the bounded, revalidated recovery path. The action fails closed for both error values. |
