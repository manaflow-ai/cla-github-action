...
jobs:
  ...
  CLAAssistant:
    ...
    outputs:
      ...
      api_result: ${{ steps.cla_action.outputs.api_result }}
    runs-on: ubuntu-latest
    timeout-minutes: 10
    permissions:
...
The action also emits `api_result` as a small, stable result type for trusted downstream policy jobs. It is `success` when the action completes without a policy failure, `unsigned` when GitHub requests completed but one or more contributors still need to sign, `retryable_error` when a known transport or transient GitHub API failure prevented a decision, and `error` for unknown status-less failures, validation, authorization, malformed-response, or permanent API failures. Both error values fail the action and leave `cla_passed=false`; `retryable_error` is a classification only. Diagnostic text is bounded and extracts only a safe message, error name, error code, and validated HTTP status. URLs, bearer credentials, headers, and nested request options are redacted. The action does not retry a failed API write automatically because a lost response could have committed it. Its existing bounded contents-conflict recovery is separate: it reads the ledger again, revalidates the Pull Request and signing comment, and retries only an explicit 409 conflict.
...
| Name                  | Description |
| --------------------- | ----------- |
| `api_result` | Stable result type for trusted consumers: `success`, `unsigned`, `retryable_error`, or `error`. A retryable result identifies a known transport or transient GitHub API failure and does not authorize a retry by itself. Unknown status-less failures are terminal `error` values. Failed API writes are not retried automatically; only an explicit 409 contents conflict uses the bounded, revalidated recovery path. Diagnostics are bounded and redact URLs, credentials, headers, and request options. The action fails closed for both error values. |
